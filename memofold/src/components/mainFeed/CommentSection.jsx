...
const CommentSection = ({
    ...
    activeReplyInputs,
    fetchComments, // <-- Pass fetchComments from MainFeed
}) => {
    const observer = useRef();
    const loadMoreRef = useRef();

    useEffect(() => {
        if (!post.commentsNextCursor || loadingComments[post._id]) return;
        ...
        observer.current = new window.IntersectionObserver(
            (entries) => {
                if (
                    ...
                    fetchComments(post._id, post.commentsNextCursor, true);
                }
            },
            { threshold: 1 }
        );
    ...
    }, [post.commentsNextCursor, loadingComments[post._id]]);
            ...
            <div className="mt-4">
                {post.comments && post.comments.length > 0 ? (
                    ...
                    >
                        ...
                        {!post.commentsNextCursor && post.comments.length > 0 && (
