...
const ConfirmationModal = ({
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* Modal Container - No backdrop, just centered */}
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`flex-shrink-0 p-3 rounded-full ${isDarkMode ? 'bg-gray-700' : 'bg-red-50'}`}>
                ...
                <div className="flex flex-col sm:flex-row sm:justify-end sm:space-x-3 space-y-3 sm:space-y-0">
                  ...
                  <button
                    onClick={onConfirm}
                    disabled={isLoading}
                    className={`px-5 py-2.5 rounded-lg font-medium text-white transition-all duration-200 ${getConfirmButtonColor()} cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 focus:outline-none focus:ring-2`}
                  >
                    {isLoading ? (
                      <>
